...
def main(args):
	...
	with tf.variable_scope('input_reader'):
		...
		left_img_batch, right_img_batch, l_disp_image_batch, r_disp_image_batch, gt_image_batch = data_set.get_batch()
		inputs={
			'left':left_img_batch,
			'right':right_img_batch,
			'l_disp':l_disp_image_batch,
			'r_disp':r_disp_image_batch,
	...
	with tf.variable_scope('model'):
		...
		net_args['left_disp'] = l_disp_image_batch
		net_args['right_disp'] = r_disp_image_batch
		net_args['fusion'] = args.fusion
	...
	if args.mode == 'MAD':
		...
		inputs_modules = {
			...
			'l_disp': scale_tensor(l_disp_image_batch, args.reprojectionScale) / args.reprojectionScale,
			'r_disp': scale_tensor(r_disp_image_batch, args.reprojectionScale) / args.reprojectionScale,
	...
	with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
		...
		print("(before restore) model/gc-read-pyramid/conv1/weights : ", sess.run('model/gc-read-pyramid/conv1/weights:0'))
...
if __name__=='__main__':
	...
	parser.add_argument("--fusion", default="None", choices=["input_fusion","None"])
