...
class MadNet(Stereo_net.StereoNet):
    _valid_args = [
        ...
        ("left_disp", "left disparity batch"),
        ("right_disp", "right disparity batch"),
    ...
    def _preprocess_inputs(self, args):
        ...
        #input fusion
        if self.fusion == "input_fusion":
            self._left_disp_batch = args['left_disp']
            self._left_disp_batch = tf.cast(self._left_disp_batch, tf.float32)
            self._right_disp_batch = args['right_disp']
            self._right_disp_batch = tf.cast(self._right_disp_batch, tf.float32)
            # self._left_input_batch = tf.concat([self._left_input_batch, self._left_disp_batch], -1)
            # self._right_input_batch = tf.concat([self._right_input_batch, self._right_disp_batch], -1)
    ...
    def _pyramid_features(self, input_batch, input_disp=None,  scope='pyramid', reuse=False, layer_prefix='pyramid'):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            if self.fusion == "input_fusion":
                # mlp to create fusion feature
                # TODO: Sparse disp 중 valid한 값만 뽑아서 mlp 돌린 후 원래 자리에 넣어주기
                # 1.	Sparse_disp 중 valid한 값으로 1d-array 만들기
                # 2.	Disp로 mlp 돌려서 feature 뽑기
                # 3.	원래 있던 자리에 넣어주기 (using tf.sparsetensor
                names.append('{}/dense1'.format(layer_prefix))
                valid_indices =  tf.where(input_disp>0)#tf.boolean_mask(input_disp, input_disp > 0)
                print(valid_indices.get_shape())
                valid_disp = tf.boolean_mask(input_disp, valid_indices)
                input_layer = tf.layers.flatten(valid_disp)
                self._add_to_layers(names[-1], sharedLayers.dense(input_layer, [input_layer.get_shape()[-1],input_layer.get_shape()[-1]]))

                # conv1_1 (1x1 convolution)
                names.append('{}/conv1_1'.format(layer_prefix))
                input_layer = tf.concat([input_batch, tf.reshape(self._get_layer_as_input(names[-2]), input_disp.get_shape())], -1)
                self._add_to_layers(names[-1], sharedLayers.conv1_1(input_layer, [1, 1, input_layer.get_shape()[-1].value, 3], strides=1, name='conv1_1', bName='biases', activation=activation))
                # conv1
                names.append('{}/conv1'.format(layer_prefix))
                input_layer = self._get_layer_as_input(names[-2])
                self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [3, 3, 3, 16], strides=2, name='conv1', bName='biases', activation=activation))
                # self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [3, 3, 4, 16], strides=2, name='conv1', bName='biases', activation=activation))
            else:
                # conv1
                names.append('{}/conv1'.format(layer_prefix))
                input_layer = input_batch
                self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [3, 3, input_batch.get_shape()[-1].value, 16], strides=2, name='conv1',
                                                                   bName='biases', activation=activation))
    ...
    def _build_network(self, args):
        ...
        if self.fusion == 'input_fusion':
            ...
            self._pyramid_features(self._left_input_batch, self._left_disp_batch, scope='gc-read-pyramid', layer_prefix='left')
            # setup pyramid features right
            self._pyramid_features(self._right_input_batch, self._right_disp_batch, scope='gc-read-pyramid', reuse=True, layer_prefix='right')
        else:
            # setup pyramid features left
