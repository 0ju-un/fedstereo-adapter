...
def dense(x, kernel_shape, wName='weights', activation=lambda x: tf.maximum(0.1 * x, x),bName='bias', name='fc', reuse=False):
    with tf.variable_scope(name, reuse=reuse):
        W = tf.get_variable(wName, kernel_shape, initializer=INITIALIZER_DENSE)
        b = tf.get_variable(bName, kernel_shape[-1], initializer=INITIALIZER_BIAS)
        x = tf.nn.xw_plus_b(x,W,b)
        x = activation(x)
        return x
def conv1_1(x, kernel_shape, strides=1, activation=lambda x: tf.maximum(0.1 * x, x), padding='SAME', name='conv1d', reuse=False, wName='weights',  bName='biases', batch_norm=False, training=False):
    with tf.variable_scope(name, reuse=reuse):
        W = tf.get_variable(wName, kernel_shape, initializer=INITIALIZER_CONV)
        b = tf.get_variable(bName, kernel_shape[3], initializer=INITIALIZER_BIAS)
        x = tf.nn.conv2d(x, W, strides=[1, strides, strides, 1], padding=padding)
