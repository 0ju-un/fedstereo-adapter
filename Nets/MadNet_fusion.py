...
class MadNet_fusion(Stereo_net.StereoNet):
    _valid_args = [
        ("left_img", "meta op for left image batch"),
        ("right_img", "meta op for right image batch"),
                      ("proxy_l", "left proxy image batch"),
                      ("s_proxy", "right proxy image batch"),
                      ("s_proxy_r", "right proxy image batch"),
    ...
    _netName = "MADNet_fusion"

    def __init__(self, **kwargs):
        """
        Creation of a MadNet for stereo prediction
        """
        super(MadNet_fusion, self).__init__(**kwargs)
    ...
    def _validate_args(self, args):
        ...
        """
        super(MadNet_fusion, self)._validate_args(args)
    ...
    def _preprocess_inputs(self, args):
        ...
        # TODO: preprocess proxy
        self._left_proxy_batch = args['proxy_l']
        self._left_proxy_batch = tf.cast(self._left_proxy_batch, tf.float32)
        # self._right_proxy_batch = args['proxy_r']
        # self._right_proxy_batch = tf.cast(self._right_proxy_batch, tf.float32)
        self._left_sparse_proxy_batch = args['s_proxy']
        self._right_sparse_proxy_batch = args['s_proxy_r']
    ...
    def _pyramid_features(self, input_batch, sparse_batch, scope='pyramid', reuse=False, layer_prefix='pyramid'):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            names.append('{}/conv1_2'.format(layer_prefix))
            input_layer = self._get_layer_as_input(names[-2])
            self._add_to_layers(names[-1], sharedLayers.conv1d(input_layer, [1, input_layer.get_shape(
            )[-1].value, 3], stride=1, name='conv1_2', bName='biases', activation=activation))
            ...
            input_layer = tf.concat([input_batch,img_pgt],-1)
            self._add_to_layers(names[-1], sharedLayers.conv2d(input_layer, [1, 1, input_layer.get_shape(
            )[-1].value, 3], strides=1, name='conv1_3', bName='biases', activation=activation))
    ...
    def _build_network(self, args):
        ...
        self._pyramid_features(self._left_input_batch, self._left_sparse_proxy_batch, scope='gc-read-pyramid', layer_prefix='left')
        # setup pyramid features right
        self._pyramid_features(self._right_input_batch, self._right_sparse_proxy_batch, scope='gc-read-pyramid', reuse=True, layer_prefix='right')
